...
function Login() {
  // ... (tu código de Login aquí, no necesita cambios)
...
// --- COMPONENTE APP (AHORA SOLO MANEJA LA LÓGICA DE SESIÓN) ---
export default function App() {
  ...
  // Si hay sesión, mostramos el Header y el Scheduler principal
  return (
    <div>
      <Header />
...
// --- COMPONENTE SCHEDULER (AHORA EXPORTADO) ---
export function Scheduler({ userId }: { userId: string }) {
  // ... (todo tu código de Scheduler aquí, no necesita cambios)
...
// --- COMPONENTE CREATEDIALOG (SIN CAMBIOS) ---
function CreateDialog({ startISO, onClose, onCreated, therapistId }: { startISO: string; onClose: () => void; onCreated: () => void; therapistId: string; }) {
  // ... (tu código de CreateDialog aquí, no necesita cambios)
  ...
  async function save() {
    ...
    try {
      ...
      if (name.trim()) {
        const { data: existing } = await supabase.from('patients').select('id').eq('therapist_id', therapistId).ilike('full_name', name.trim()).limit(1);
        if (existing && existing.length) {
          patientId = existing[0].id;
        } else {
          const { data: p, error: pe } = await supabase.from('patients').insert({ therapist_id: therapistId, full_name: name.trim(), phone }).select('id').single();
          if (pe) throw pe;
          patientId = p!.id;
        }
      }
      const { error: ae } = await supabase.from('appointments').insert({ therapist_id: therapistId, patient_id: patientId || null, start_at: startISO, end_at: endISO, status: 'scheduled', service, note, created_by: therapistId });
      if (ae) throw ae;
      onCreated();
    } catch (e) {
