...
const TZ = import.meta.env.VITE_TZ || 'America/Montevideo';
...
function Scheduler({ userId }: { userId: string }) {
  ...
  const [date, setDate] = useState(() => (window as any).dayjs?.().startOf('day') || dayjs().startOf('day'));
  ...
  const dayEnd   = date.hour(20).minute(0).second(0);
  const slots = range30(dayStart, dayEnd);
  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-4">
      <header className="flex items-center justify-between gap-2">
        <button className="px-3 py-2 rounded-xl border" onClick={()=>setDate((d:any)=>d.add(-1,'day'))}>◀︎</button>
      ...
      {creating && (
        <CreateDialog
          ...
          onCreated={() => { setCreating(null); setDate((d:any)=>d.clone()); }}
          therapistId={userId}
        />
      )}
...
function CreateDialog({ startISO, onClose, onCreated, therapistId }:{ startISO:string; onClose:()=>void; onCreated:()=>void; therapistId:string; }) {
  ...
  async function save() {
    setSaving(true);
    try {
      // 1) crear/obtener paciente por nombre simple (para MVP)
