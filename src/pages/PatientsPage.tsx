        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-sm rounded-xl border bg-blue-600 text-white hover:bg-blue-700"
            >
              Nuevo Paciente
            </button>
          </div>
          ...
          <div className="md:col-span-2">
            {selectedPatient ? (
              <PatientFile
                ...
                onUpdateAppointments={() => fetchAppointments(selectedPatient.id)}
              />
            ) : (
...
// --- MODIFICADO: Diccionario para traducir las claves del historial ---
const historyKeyMap: { [key: string]: string } = {
  allergies: "Alergias",
  surgeries: "Operaciones",
  diagnosed_illnesses: "Enfermedades diagnosticadas",
  physical_problems: "Problemas físicos",
  emotional_mental_problems: "Problemas emocionales/mentales",
  children: "Hijos",
  partner: "Pareja",
  other: "Otros",
};
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("appointments")
        .update({
          visit_observations: editing.text,
          byosen_points: editing.points,
        })
        .eq("id", editing.id);
    ...
    <div className="bg-white p-4 rounded-xl border space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Tel: {patient.phone || "N/A"} | Email: {patient.email || "N/A"}
          </p>
          <p className="text-sm text-gray-600">
            F.N.:{" "}
            {patient.birth_date
      ...
      <section className="space-y-2 text-sm">
        ...
        <p className="p-2 bg-gray-50 rounded-md whitespace-pre-wrap">
          {patient.consultation_reason || "No especificado"}
        </p>
      </section>
      ...
      <section className="space-y-2 text-sm">
        ...
        <div className="p-2 bg-gray-50 rounded-md">
          {Object.entries(patient.medical_history || {}).length === 0 && (
            <p className="text-gray-500">Sin datos.</p>
          )}
          {/* --- MODIFICADO: Usamos el diccionario para traducir --- */}
          {Object.entries(patient.medical_history || {}).map(([key, value]) =>
            value ? (
              <p key={key}>
                <b>{historyKeyMap[key] || key}:</b> {value}
              </p>
            ) : null
          )}
        </div>
          ...
          {appointments.map((appt) => (
            <div key={appt.id} className="p-3 border rounded-lg">
              <p className="font-semibold capitalize">
                {dayjs(appt.start_at).format("dddd, D MMM YYYY - HH:mm [hs]")}
              </p>
              <p className="text-sm">
                <b>Servicio:</b> {appt.service || "N/A"}
              </p>
              <div className="mt-2">
                <h4 className="text-xs font-semibold text-gray-600">
                  Observaciones y Lámina Byosen
                </h4>
                {editing?.id === appt.id ? (
                  <div className="mt-1">
                    <textarea
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button
                        onClick={() =>
                          setEditing({
                            ...editing,
                            points: editing.points.slice(0, -1),
                          })
                        }
                        className="text-xs px-2 py-1 rounded border"
                  ...
                  <div
                    onClick={() =>
                      setEditing({
                        id: appt.id,
                        text: appt.visit_observations || "",
                        points: (appt.byosen_points as ByosenPoint[]) || [],
                      })
                    }
                    className="cursor-pointer hover:bg-gray-50 rounded p-1"
                  >
                    <p className="text-sm whitespace-pre-wrap">
                      {appt.visit_observations || (
                        <span className="text-gray-400">
                          Clic para añadir observaciones…
                        </span>
                      )}
                    </p>
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="w-full max-w-lg bg-white rounded-2xl p-5 space-y-4">
        <h2 className="text-lg font-semibold">
          {patient ? "Editar Ficha" : "Nuevo Paciente"}
        </h2>
        ...
        <textarea
          ...
          onChange={(e) =>
            setForm({ ...form, consultation_reason: e.target.value })
          }
        />

        <h3 className="font-semibold text-sm pt-1">
          Historial / Cuestiones de Interés
        </h3>
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <input
            ...
            value={history.diagnosed_illnesses || ""}
            onChange={(e) =>
              setHistory({
                ...history,
                diagnosed_illnesses: e.target.value,
              })
            }
          />
          <input
            className="border rounded-xl px-3 py-2"
            placeholder="Problemas físicos"
            value={history.physical_problems || ""}
            onChange={(e) =>
              setHistory({
                ...history,
                physical_problems: e.target.value,
              })
            }
          />
          <input
            className="border rounded-xl px-3 py-2"
            placeholder="Problemas emocionales/mentales"
            value={history.emotional_mental_problems || ""}
            onChange={(e) =>
              setHistory({
                ...history,
                emotional_mental_problems: e.target.value,
              })
            }
          />
          <input
            ...
            onChange={(e) =>
              setHistory({ ...history, children: e.target.value })
            }
          />
          <input
            className="border rounded-xl px-3 py-2"
            ...
            onChange={(e) =>
              setHistory({ ...history, partner: e.target.value })
            }
          />
          <input
            className="border rounded-xl px-3 py-2 col-span-2"
            ...
            value={history.other || ""}
            onChange={(e) =>
              setHistory({ ...history, other: e.target.value })
            }
          />
        </div>
